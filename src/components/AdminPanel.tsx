    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-6 flex items-center justify-between group">
           <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${globalSettings?.maintenanceMode ? 'bg-red-500/20 text-red-500' : 'bg-neon-green/20 text-neon-green'}`}>
                <Power size={24} />
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-white/40">Modo Manutenção</p>
                <h4 className={`text-xl font-display font-black italic ${globalSettings?.maintenanceMode ? 'text-red-500 underline decoration-red-500/30' : 'text-neon-green'}`}>
           ...
           <button 
            ...
            className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${globalSettings?.maintenanceMode ? 'bg-neon-green text-black px-6' : 'bg-red-500 text-white'}`}
           >
             {globalSettings?.maintenanceMode ? 'Abrir App' : 'Fechar App'}
           </button>
        </div>

        <div className="glass-card p-6 bg-neon-purple/5 border-neon-purple/20">
           <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-neon-purple/10 flex items-center justify-center text-neon-purple">
                <Megaphone size={24} />
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-white/40">Status Anúncio</p>
                <h4 className="text-xl font-display font-black italic text-white flex items-center gap-2">
                  {globalSettings?.showAnnouncement ? (
                    <>Ativo <div className="w-2 h-2 bg-neon-purple rounded-full animate-pulse" /></>
                  ) : 'Nenhum'}
                </h4>
              </div>
           </div>
      ...
      <div className="glass-card p-8 border-neon-purple/20">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <textarea 
            ...
            className="flex-1 bg-white/5 border border-white/10 rounded-xl p-4 text-sm focus:border-neon-purple outline-none min-h-[100px] resize-none"
          />
          <div className="flex flex-col gap-3 md:w-48">
            <button 
              onClick={handlePostAnnouncement}
              className="w-full h-12 bg-neon-purple text-black font-black uppercase tracking-widest text-[10px] rounded-lg shadow-[0_10px_20px_rgba(188,19,254,0.3)] hover:scale-105 active:scale-95 transition-all"
            >
              Postar Anúncio
            </button>
            <button 
              onClick={handleClearAnnouncement}
              className="w-full h-12 bg-white/5 border border-white/10 text-white/40 font-black uppercase tracking-widest text-[10px] rounded-lg hover:text-white transition-all"
            >
              Remover Atual
            </button>
          </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-white/[0.02] text-[10px] uppercase font-black tracking-widest text-white/40">
              <tr>
                ...
                <th className="px-8 py-4">Saldo Real</th>
                ...
                <tr key={u.id} className="hover:bg-white/[0.01] transition-colors group">
                  <td className="px-8 py-5">
                    <div className="flex flex-col">
                      <span className="text-sm font-bold text-white mb-0.5">{u.email}</span>
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-2">
                       ...
                       <button 
                        ...
                        className="p-1.5 bg-white/5 text-white/20 hover:text-white rounded-md transition-all opacity-0 group-hover:opacity-100"
                       >
                         <Wallet size={14} />
                       </button>
                    </div>
